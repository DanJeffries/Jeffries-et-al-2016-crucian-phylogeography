...
mycolsolid <- c("darkgreen", "#CCFF00FF",  "blue", "purple", "darkred",  "red", "#FF6600FF","green","goldenrod", "lightgreen", "turquoise", "brown" )

mycol <- adjustcolor(mycolsolid, alpha = .6)

setwd("C:/Users/djeffrie/Desktop/Microsatellites/DAPC/Complete dataset outputs/")

map("worldHires", xlim=c(-10, 45), ylim=c(43,70), col="gray90", fill=TRUE)##plots the area of the map that I am interested in (just Europe, leave out x/ylim arguments for whole world view)

cords <- read.csv("C:/Users/djeffrie/Desktop/Microsatellites/R stats/Adegenet IBD/Mantelcoordinates.csv", header = T)  ## load my microsat coordinates file. 

geocent <- read.csv("C:/Users/djeffrie/Desktop/Microsatellites/R stats/mapplots/geo cluster centre.csv")  ##  Loads my geographic centre data

points(geocent$lat, geocent$lon, pch=19, col=mycol, cex=3)  ## Plots the geocent data on the map.

mtcords <- read.csv("C:/Users/djeffrie/Desktop/Microsatellites/R stats/Adegenet IBD/MtDNAcoordinates.csv", header = T)  ## Loads my mtDNA sample coordinates

points(cords$lon, cords$lat, pch=19, col="black", cex=0.8)  ## Plots my microsat sample locations on the map 

points(mtcords$lat, mtcords$lon, pch=19, col="blue", cex=0.8)  ## Plots my mtDNA Genbank sample locations on the map 
...
Clustermemberships <- (dapc1$posterior)

write.csv(Clustermemberships, file = "C:/Users/Dan/Dropbox/PhD/Dan's PhD (Shared)/Data/Microsatellites/DAPC/Outputs/Final Run/ClustermembershipsbodnoSTEC.csv")  ## Wrote cluster memberships to csv file.

## Note, if you are carrying on straight from DAPC analyses you can obviously skip this writing and reading process. 


body <- read.csv("C:/Users/djeffrie/Desktop/Microsatellites/DAPC/Complete dataset outputs/Whole EU 4 clusters/WholeEU_Clustermemberships.csv", header = T)  ## Load my cluster membership data file.
head(body)

names(body) = c('population','cluster1', 'cluster2', 'cluster3','cluster4')

## Next create an object containing the averaged cluster memberships for each subset. 



WholeEU_meanclustermembs <- sapply(split(body[2:5],body$population),colMeans) ##Very handy script, "colMeans" calculates the mean of columns in the subgroups produced as a result of performing split(body[2:13],body$population).  I then wrote this to the .csv below.

WholeEU_meanclustermembs  ## check it 

write.csv(WholeEU_meanclustermembs, "C:/Users/Dan/Dropbox/PhD/Dan's PhD (Shared)/Data/Microsatellites/DAPC/Complete dataset outputs/Whole EU 4 clusters/WholeEU_mean_clustermemberships.csv")   ## I manually added my population names and cloumn headers to the csv 

##Just add the below script after plotting the map.

pies <- read.csv("C:/Users/djeffrie/Desktop/Microsatellites/DAPC/Complete dataset outputs/Whole EU 4 clusters/WholeEU_mean_clustermemberships.csv", header=T) ## have read this in again and specified that there are headers as I was having difficulty assigning headers to the object. This allows me to call populattions using the $ operator as below.

## Get the population names for the structure outputs

names <- read.delim("C:/Users/djeffrie/Desktop/STRUCTURE/Names.txt", header = F)

popnames <- unique(names$V1)

data.frame(popnames)

structure <- read.delim("C:/Users/djeffrie/Desktop/STRUCTURE/All_data_1-12-15/all_data_1-12-15/K11_cluter_memb_probs.txt", header = F)

structure$X <- NULL

row.names(structure) <- popnames

tstructure <- t(structure)

pies <- as.data.frame(tstructure)
